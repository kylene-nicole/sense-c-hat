...
# Set up logging - edi tthis for username later
log_file = "/home/$(whoami)/sense-c-hat/logs/imu.log"
...
class IMU(object):
  def __init__(self):
    try:
      ...
      if self._read_byte(I2C_ADD_IMU_QMI8658,0x00) != 0x05:
        logger.error("QMI8658_init fail\n")
        raise Exception("QMI8658_init fail\n")
      else:
        logger.error("QMI8658_init slave=0x%x  QMI8658Register_WhoAmI=0x%x 0x%x\n" % (I2C_ADD_IMU_QMI8658,self._read_byte(I2C_ADD_IMU_QMI8658,0x00),self._read_byte(I2C_ADD_IMU_QMI8658,0x01)))
      ...
      if self._read_byte(I2C_ADD_IMU_AK09918,AK09918_WIA2) != 0x0C:
        logger.error("AK09918 init fail\r\n")
        return 0
      else:
        logger.error("AK09918 init Success\r\n")
  ...
  def _read_byte(self, address, cmd):
    try:
        ...
        logger.error("Error reading byte from I2C: %s", str(e))
        raise
  ...
  def _write_byte(self, address, cmd, val):
    try:
        ...
        logger.error("Error writing byte to I2C: %s", str(e))
        raise
